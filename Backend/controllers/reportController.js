...
exports.createReport = async (req, res) => {
  try {
    ...
    if (!title || !periodStart || !periodEnd) {
      return res.status(400).json({ message: 'Title, periodStart, and periodEnd are required.' });
    }
...
// Get a single report by ID (with daily inputs included)
exports.getReportById = async (req, res) => {
  try {
    const report = await Report.findById(req.params.id).lean();
    if (!report) return res.status(404).json({ message: 'Report not found' });

    // Optionally include daily inputs
    const dailyInputs = await DailyInput.find({ reportId: req.params.id });
    res.json({ ...report, dailyInputs });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
exports.summarizeWeeklyInputs = async (req, res) => {
  try {
    ...
    if (!inputTexts.length) {
      return res.status(400).json({ message: 'No daily inputs found for this report.' });
    }
